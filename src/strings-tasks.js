...
function repeatString(str, times) {
  if (typeof times === 'number' && times > 0 && Number.isInteger(times)) {
    return typeof str === 'string' ? str.repeat(times) : str;
  }
  return '';
}
...
function countVowels(str) {
  if (typeof str === 'string' && str !== null) {
    ...
    const count = [...str].filter((item) => vowels.includes(item)).length;
    return count;
  }
  return 0;
}
...
function findLongestWord(sentence) {
  if (typeof sentence === 'string' && sentence !== null) {
    const words = sentence.split(' ');
    let longestWord = '';

    words.forEach(function getWord(word) {
